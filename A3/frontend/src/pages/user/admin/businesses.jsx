  ...
  return (
    <div className="flex items-center justify-between gap-4 text-sm">
      ...
      <div className="flex gap-2">
        ...
        <Button type="button" variant="outline" size="sm" onClick={() => onPageChange(page + 1)} disabled={page >= totalPages}>Next</Button>
      </div>
    </div>
  )
}
    ...
    <div className="mx-auto max-w-7xl px-6 py-10">
      <div className="mb-8 flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-muted-foreground">Search, sort, filter, paginate, and verify businesses.</p>
        </div>
      ...
      <Card className="mb-6">
        <CardHeader>
          ...
          <CardDescription>Matches the API for <code>GET /businesses</code> when called by an admin.</CardDescription>
        </CardHeader>
        <CardContent>
          <form className="grid gap-4 md:grid-cols-4" onSubmit={applyFilters}>
            ...
            <div className="space-y-2">
              ...
              <select className="flex h-10 w-full rounded-3xl border border-input bg-background px-3 py-2 text-sm" value={draft.order} onChange={(event) => setDraft((current) => ({ ...current, order: event.target.value }))} disabled={!draft.sort}>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {isLoading ? (
            ...
            <div className="py-8 text-center text-sm text-muted-foreground">No businesses matched the current filters.</div>
          ) : (
            <>
                  ...
                  <tbody>
                    {data.results.map((business) => (
                      <tr key={business.id} className="border-b last:border-b-0">
                        ...
                        <td className="px-3 py-4"><Badge variant={business.activated ? "default" : "outline"}>{business.activated ? "Activated" : "Inactive"}</Badge></td>
                        <td className="px-3 py-4"><Badge variant={business.verified ? "default" : "secondary"}>{business.verified ? "Verified" : "Pending"}</Badge></td>
